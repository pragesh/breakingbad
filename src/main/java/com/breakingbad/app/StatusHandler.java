...
public class StatusHandler implements ISVNStatusHandler, ISVNEventHandler {
	...
	private List<String> conflictedFileName = new ArrayList<String>();
	public StatusHandler(boolean isRemote,List<String> files) {
		myIsRemote = isRemote;
		this.conflictedFileName = files;
	}
	...
	public void handleStatus(SVNStatus status) {
		...
		if (contentsStatus == SVNStatusType.STATUS_MODIFIED) {
			 ...
			 */
			pathChangeType = "C";
			if (!conflictedFileName.contains(status.getFile().getName())){
				conflictedFileName.add(status.getFile().getPath());
			}
